...
const (
	DefaultWriteInterval = 15 * time.Minute
...
func (w *Worker) handle(parentCtx context.Context, t *time.Ticker) (bool, error) {
	select {
	case <-parentCtx.Done():
		return true, nil
	case status := <-w.terminateWithStatus:
		ctx, cancel := context.WithTimeout(parentCtx, WriteTimeout)
		defer cancel()
