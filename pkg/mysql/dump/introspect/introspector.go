...
func (i *Introspector) getTables(ctx context.Context, tx interfaces.DB) ([]models.Table, error) {
	...
	defer func() {
		if err := rows.Close(); err != nil {
			log.Ctx(ctx).Warn().Err(err).Msg("error rows")
		}
	}()
	var tableIDSeq int
...
func (i *Introspector) getColumns(ctx context.Context, tx interfaces.DB, tableSchema string, tableName string) ([]models.Column, error) {
	query := `
