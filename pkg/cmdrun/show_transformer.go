	...
	for _, p := range def.Parameters {
		...
		if p.IsColumnContainer {
			...
			if containerProp != nil && containerProp.ColumnProperties != nil {
				if len(containerProp.AllowedTypes) > 0 {
					types = containerProp.AllowedTypes
				}
				if len(containerProp.AllowedTypeClasses) > 0 {
					typeClasses = make([]string, len(containerProp.AllowedTypeClasses))
					for i, tc := range containerProp.AllowedTypeClasses {
