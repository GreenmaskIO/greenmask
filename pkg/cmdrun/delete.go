...
func RunDelete(cfg *config.Config, opts DeleteOptions) error {
	...
	case DeleteModeRetainFor:
		if err := retainForDumps(ctx, cfg, st, opts.RetainFor, opts.DryRun); err != nil {
			return fmt.Errorf("retain for dumps: %w", err)
		}
	case DeleteModeRetainRecent:
		if err := retainRecentNDumps(ctx, cfg, st, opts.RetainRecent, opts.DryRun); err != nil {
			return fmt.Errorf("retain recent dumps: %w", err)
		}
	case DeleteModePruneFailed:
		if err := pruneFailedDumps(ctx, cfg, st, opts.PruneUnsafe, opts.DryRun); err != nil {
			return fmt.Errorf("prune failed dumps: %w", err)
		}
	case DeleteModeBeforeDate:
		if err := deleteBeforeDate(ctx, cfg, st, opts.BeforeDate, opts.DryRun); err != nil {
...
func pruneFailedDumps(
	...
	sr, err := getSortedBackupWithStatuses(ctx, cfg, st)
	if err != nil {
...
func getSortedBackupWithStatuses(ctx context.Context, cfg *config.Config, st interfaces.Storager) (*StorageResponse, error) {
	...
	for _, backup := range backups {
		...
		case DumpStatusInProgress:
			log.Ctx(ctx).Debug().
				Str("DumpID", backup.Dirname()).
				Msg("dump status is in progress: skipping it in delete operations")
		default:
