...
) (map[toolkit.Oid]*dump_objects.Table, toolkit.ValidationWarnings, error) {
	result := make(map[toolkit.Oid]*dump_objects.Table, len(cfg))
...
func getTable(ctx context.Context, tx pgx.Tx, t *domains.Table) ([]*dump_objects.Table, toolkit.ValidationWarnings, error) {
	table := &dump_objects.Table{
		Table: &toolkit.Table{},
	}
	var warnings toolkit.ValidationWarnings
	var tables []*dump_objects.Table
