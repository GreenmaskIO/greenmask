...
// Deprecated
func GetPgTypeAndEncodingPlan(typeMap *pgtype.Map, typeOid data_section.Oid, castVal any) (*pgtype.Type, pgtype.EncodePlan, error) {
...
func Scan(src string, dest interface{}, oid uint32, typeMap *pgtype.Map, pgType *pgtype.Type) error {
	...
	if err != nil {
		return fmt.Errorf("cannot scan min value: %w", err)
	}
