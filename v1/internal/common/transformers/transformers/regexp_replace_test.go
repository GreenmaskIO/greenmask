...
func TestRegexpReplaceTransformer_Transform2(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			original: []*commonmodels.ColumnRawValue{
				commonmodels.NewColumnRawValue([]byte("Hello world!!!?"), false)},
			staticParameters: map[string]commonmodels.ParamsValue{
				"column":  commonmodels.ParamsValue("data"),
				"regexp":  commonmodels.ParamsValue(`(Hello)\s*world\s*(\!+\?)`),
				"replace": commonmodels.ParamsValue("$1 Mr NoName $2"),
			},
			validateFn: func(t *testing.T, recorder commonininterfaces.Recorder) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			env := newTransformerTestEnvReal(t,
				RegexpReplaceTransformerDefinition,
				tt.columns,
				tt.staticParameters,
				tt.dynamicParameter,
			)
