...
import (
	...
	"github.com/greenmaskio/greenmask/v1/pkg/common/subset/tablegraph"
)
...
func NewSubset(tables []commonmodels.Table, dialect Dialect) (Subset, error) {
	...
	condensationGraph := condensationgraph2.NewGraph(tableGraph)
	s := Subset{
		tables:            tables,
...
func dumpEdgesIntoGraph(graph *subsetGraph, edges []condensationgraph2.Edge) {
	for _, e := range edges {
		graph.addEdge(e)
	}
}
