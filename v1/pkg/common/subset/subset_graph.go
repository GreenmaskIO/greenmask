...
import (
	...
	condensationgraph2 "github.com/greenmaskio/greenmask/v1/pkg/common/subset/condensationgraph"
)
...
func newSubsetGraph(rootVertex int) *subsetGraph {
	return &subsetGraph{
		graph:      make(map[int][]condensationgraph2.Edge),
		rootVertex: rootVertex,
		vertexes:   make(map[int]condensationgraph2.SCC),
	}
}
...
func (g *subsetGraph) validateEdgeIsUnique(e condensationgraph2.Edge) error {
	...
	found := slices.ContainsFunc(existingEdges, func(edge condensationgraph2.Edge) bool {
		return edge.ID() == e.ID()
	})
	if found {
...
func (g *subsetGraph) addVertex(v int, scc condensationgraph2.SCC) {
	g.graph[v] = nil
	g.vertexes[v] = scc
}

// addEdge - adds the edge to the graph.
func (g *subsetGraph) addEdge(e condensationgraph2.Edge) {
