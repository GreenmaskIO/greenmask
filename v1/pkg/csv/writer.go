...
func indexAnyByte(b, targets []byte) int {
	for i := 0; i < len(b); i++ {
...
func isSymbolSpecial(b byte) bool {
	return b == '"' || b == '\r' || b == '\n'
}
...
func (w *Writer) Write(record [][]byte) error {
	...
	for n, field := range record {
		...
		for len(field) > 0 {
			...
			i := slices.IndexFunc(field, isSymbolSpecial)
			if i < 0 {
				i = len(field)
			}
...
func (w *Writer) fieldNeedsQuotes(field []byte) bool {
	...
	// Quote special PostgreSQL terminator
	...
	if w.Comma < utf8.RuneSelf {
		for i := 0; i < len(field); i++ {
			c := field[i]
			if c == '\n' || c == '\r' || c == '"' || rune(c) == w.Comma {
				return true
			}
		}
	} else {
		// UTF-8 Comma or wide character
		for len(field) > 0 {
			r, size := utf8.DecodeRune(field)
			if r == w.Comma || r == '"' || r == '\n' || r == '\r' {
				return true
			}
			field = field[size:]
