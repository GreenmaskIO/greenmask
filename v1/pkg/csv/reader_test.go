...
func castToBytes(v [][]byte) []string {
	...
	for _, tt := range readTests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			if wantErr := firstError(tt.Errors, positions, errPositions); wantErr != nil {
				...
				if !reflect.DeepEqual(byteRecordsToStringRecords(out), tt.Output) {
			...
			for recNum := 0; ; recNum++ {
				...
				if got, want := rec, tt.Output[recNum]; !slices.Equal(castToBytes(got), want) {
